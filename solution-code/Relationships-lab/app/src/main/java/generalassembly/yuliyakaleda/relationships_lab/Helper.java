...
public class Helper extends SQLiteOpenHelper {
  ...
  public List<String> getCompanyWithHighestSalary () {
    ...
    SQLiteDatabase db = getReadableDatabase();
    // Building query using INNER JOIN keyword
    String query = "SELECT SALARY, FIRSTNAME, LASTNAME FROM JOB INNER JOIN EMPLOYEE ON EMPLOYEE.SSN = JOB.SSN";
    Cursor cursor = db.rawQuery(query, null);
    if(cursor.moveToFirst()) {
      while (!cursor.isAfterLast()) {
        ...
        if (highestSalary < currentSalary) {
          ...
          first = cursor.getString(cursor.getColumnIndex(DataEntryEmployee.COLUMN_FIRST_NAME));
          last = cursor.getString(cursor.getColumnIndex(DataEntryEmployee.COLUMN_LAST_NAME));
        }
        cursor.moveToNext();
      }
    }
